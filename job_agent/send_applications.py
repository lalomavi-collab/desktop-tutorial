...
# Load sent log to avoid re-sending emails
SENT_LOG = os.path.join(os.path.dirname(__file__), "sent_log.json")
try:
...
for app_data in applications:
    key = app_data["company"] + "|" + app_data["to_email"]
    if key in sent_emails:
        print("SKIP (already sent): " + app_data["company"] + " -> " + app_data["to_email"])
        continue
