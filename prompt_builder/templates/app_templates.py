"""
מודל 6 השלבים הגנרי לבניית פרומפטים.

מבנה אחיד שעובד לכל תחום - הסוכן (Claude) מתאים את השאלות
דינמית בהתאם לתחום שהמשתמש מזין.
"""
...
STEPS: list[StepDefinition] = [
    StepDefinition(
        ...
        guidance=(
            "ברר: מה התפקיד המקצועי, רמת הבכירות, תחום ההתמחות, "
            "סגנון התקשורת (רשמי/ידידותי), והאם נדרשת סמכות מקצועית מסוימת."
        ),
    ),
    StepDefinition(
        number=2,
        ...
        guidance=(
            "ברר: מי הצרכן הסופי, מה רמת הידע שלו בתחום, "
            "באיזו שפה/רמת מורכבות לכתוב, ומה הציפיות שלו מהתוצר."
        ),
    ),
    StepDefinition(
        number=3,
        ...
        guidance=(
            "ברר: איזה סוג מידע נכנס (קובץ, טקסט, API, DB), "
            "מה הפורמט, מה הגודל הצפוי, ומה קורה כשחסר מידע."
        ),
    ),
    StepDefinition(
        number=4,
        ...
        guidance=(
            "ברר: מה בדיוק צריך לעשות, מה ההגדרה של הצלחה, "
            "מה ה-edge cases, ומה קורה עם שגיאות או מקרים חריגים."
        ),
    ),
    StepDefinition(
        number=5,
        ...
        guidance=(
            "ברר: חוקים/רגולציה רלוונטיים, פורמט חובה, מגבלות אורך, "
            "טון ושפה, דברים שאסור לכלול, ורמת דיוק נדרשת."
        ),
    ),
    StepDefinition(
        number=6,
        ...
        guidance=(
            "ברר: פורמט (טבלה, רשימה, JSON, מסמך), שדות/עמודות ספציפיים, "
            "סדר המידע, מה חייב להופיע, ודוגמה לפלט רצוי."
        ),
    ),
]
