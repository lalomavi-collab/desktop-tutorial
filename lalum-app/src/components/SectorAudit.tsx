...
// The sector self-assessment: five binary questions, a score, and a gap map.
//
// Written in the app's own idiom rather than as a foreign widget: plain React,
// the CSS variables the rest of the site uses, and the shared SVG icon sprite.
// The specification for this tool asked for Tailwind, lucide-react and a navy
// and gold palette; the app has none of those, and a component that brings its
// own design system reads as a bolted-on advert rather than part of the page it
// sits in. The behaviour asked for is all here.
//
// Nothing is sent anywhere. The answers live in component state, which is what
// lets the page promise a municipal CEO that a self-assessment of their own
// exposure does not leave their browser.

type Answers = Record<string, boolean>;

const POINTS_PER_GAP = 20;

function bandFor(bands: AuditBand[], score: number): AuditBand {
  return bands.find((b) => score <= b.upTo) ?? bands[bands.length - 1];
}

// The gauge. An arc rather than a full circle, because the reading is "how far
// along a scale", and drawn in the page's own clay rather than a warning red:
// the number is the message, and colouring it like an alarm invites the reader
// to discount it.
...
export function SectorAudit({ questions, bands }: { questions: AuditQuestion[]; bands: AuditBand[] }) {
  ...
  const score = gaps.length * POINTS_PER_GAP;
    ...
    return (
      <div className="card" style={{ padding: "28px 24px", textAlign: "center" }}>
        <button type="button" className="btn btn-clay" onClick={() => setStarted(true)}>
          <Icon name="compass" size={17} /> {SECTORS_UI.auditStart}
        </button>
  ...
  return (
    <div className="card" style={{ padding: "28px 24px" }}>
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginTop: 28 }}>
        ...
        <button type="button" className="btn btn-outline" onClick={restart}>{SECTORS_UI.auditRestart}</button>
      </div>
    </div>
  );
}
