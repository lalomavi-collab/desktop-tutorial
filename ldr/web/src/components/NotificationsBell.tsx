  ...
  return (
    <div ref={ref} style={{ position: "relative" }}>
      <button onClick={() => { setOpen((o) => !o); setSeen(true); }} aria-label={t("notif.title")}
        ...
        {!seen && (
          <span style={{ position: "absolute", top: -2, insetInlineEnd: -2, minWidth: 17, height: 17, padding: "0 4px", background: "#D97757", color: "#fff", fontSize: 11, fontWeight: 700, borderRadius: 999, display: "grid", placeItems: "center", border: "2px solid #16181f" }}>{items.length}</span>
        )}
      </button>
      {open && (
        <div style={{ position: "absolute", top: 46, insetInlineEnd: 0, zIndex: 1000, width: 280, background: panelBg, border: `1px solid ${border}`, borderRadius: 14, boxShadow: "0 16px 40px rgba(0,0,0,0.3)", overflow: "hidden", backdropFilter: "blur(12px)" }}>
